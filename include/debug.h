...
void debug_str (const char8* str);

// write LMiC event name to USART
void debug_event (int ev);

// write label and 32-bit value as hex to USART
void debug_val (const char8* label, u4_t val);
void debug_dval (const char8* label, u4_t val);

void debug_opmode(const char8* label, u4_t val);
void debug_engine(const char8* str);